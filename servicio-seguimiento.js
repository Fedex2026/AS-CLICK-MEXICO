...
function renderProvider(){
  ...
  if(photo&&img){
    ...
    let photoUrl=String(photo).trim().replace(/^http:\/\//i,"https://");

    if(photoUrl.includes("res.cloudinary.com/")&&photoUrl.includes("/image/upload/")&&!photoUrl.includes("/f_jpg")){

      photoUrl=photoUrl.replace("/image/upload/","/image/upload/f_jpg,q_auto/");

    }

    img.onload=()=>{img.hidden=false;if(placeholder) placeholder.hidden=true;};

    img.onerror=()=>{img.hidden=true;if(placeholder){placeholder.hidden=false;placeholder.textContent=String(name).trim().charAt(0).toUpperCase()||"AS";}};
