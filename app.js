...
/* =========================================
   SOLICITAR SERVICIO
========================================= */
async function solicitarServicio(servicio, detalleServicio = "", confirmacionAbogado = false, datosGrua = null) {
  if (!usuarioActual) {
    mostrarModal(
      "⚠",
  ...
  const servicioNormalizado = String(servicio || "")
    ...
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "");
  if (servicioNormalizado === "grua" && !datosGrua) {
    abrirSolicitudGrua();
    return;
  }
  ...
  try {
    ...
    const mensaje = construirMensajeServicio(
      servicio,
      tipoTarifa,
      ubicacion.enlaceGoogleMaps || ubicacion.mensaje || "Ubicación no disponible",
      detalleServicio,
      folio,
      datosGrua
    );
    ...
    const enlaceSeguimientoServicio =
      generarEnlaceSeguimientoServicio(folio);
    // Conserva el envío por WhatsApp y deja la web del cliente
    // en la nueva pantalla de seguimiento.
    ...
    if (!ventanaWhatsApp) {
      try {
        sessionStorage.setItem(
          "asClickWhatsAppPendiente",
          url
        );
      } catch (errorStorage) {
        console.warn(
          "No fue posible guardar el enlace temporal de WhatsApp:",
          errorStorage
        );
      }
    }
    window.location.href = enlaceSeguimientoServicio;
  } catch (error) {
    console.error("No fue posible solicitar el servicio:", error);
    mostrarModal(
      "⚠",
      "No fue posible solicitar el servicio",
      "Revisa tu conexión, permite la ubicación e inténtalo nuevamente."
    );
  }
}
...
function abrirSolicitudGrua() {
  crearInterfazSolicitudGrua();
  const overlay = document.getElementById("solicitudGruaOverlay");
  const formulario = document.getElementById("solicitudGruaForm");
  const camposCarga = document.getElementById("camposCargaGrua");
  const error = document.getElementById("solicitudGruaError");
  if (!overlay) return;
  formulario?.reset();
  if (camposCarga) camposCarga.hidden = true;
  if (error) error.textContent = "";
  overlay.classList.add("active");
  document.body.style.overflow = "hidden";
}
function actualizarCamposCargaGrua() {
  const tieneCarga = document.querySelector(
    'input[name="gruaTieneCarga"]:checked'
  )?.value === "Sí";
  const camposCarga = document.getElementById("camposCargaGrua");
  const tipoCarga = document.getElementById("gruaTipoCarga");
  const pesoCarga = document.getElementById("gruaPesoCarga");
  if (camposCarga) camposCarga.hidden = !tieneCarga;
  if (tipoCarga) tipoCarga.required = tieneCarga;
  if (pesoCarga) pesoCarga.required = tieneCarga;
  if (!tieneCarga) {
    if (tipoCarga) tipoCarga.value = "";
    if (pesoCarga) pesoCarga.value = "";
  }
}
async function confirmarSolicitudGrua() {
  const destino = document.getElementById("gruaDestino")?.value.trim() || "";
  const condicion = document.querySelector(
    'input[name="gruaCondicion"]:checked'
  )?.value || "";
  const liberado = document.querySelector(
    'input[name="gruaLiberado"]:checked'
  )?.value || "";
  const tieneCarga = document.querySelector(
    'input[name="gruaTieneCarga"]:checked'
  )?.value || "";
  ...
  const error = document.getElementById("solicitudGruaError");
  if (!destino || !condicion || !liberado || !tieneCarga) {
    if (error) {
      error.textContent = "Completa todos los campos obligatorios.";
    }
    return;
  }
  if (tieneCarga === "Sí" && (!tipoCarga || !pesoCarga)) {
    if (error) {
      error.textContent = "Indica el tipo de carga y el peso aproximado.";
    }
    return;
  }
  if (error) error.textContent = "";
  const datosGrua = {
    ...
    liberado,
    tieneCarga,
    tipoCarga: tieneCarga === "Sí" ? tipoCarga : "",
    pesoCarga: tieneCarga === "Sí" ? pesoCarga : ""
  };
  cerrarSolicitudGrua();
  await solicitarServicio("Grúa", "", false, datosGrua);
}
function cerrarSolicitudGrua(event = null) {
  if (event && event.target.id !== "solicitudGruaOverlay") return;
  document
    .getElementById("solicitudGruaOverlay")
    ?.classList.remove("active");
  document.body.style.overflow = "";
}
function crearInterfazSolicitudGrua() {
  if (document.getElementById("solicitudGruaOverlay")) return;
  agregarEstilosFuncionesNuevas();
  const overlay = document.createElement("div");
  overlay.id = "solicitudGruaOverlay";
  overlay.className = "asClickCustomOverlay";
  overlay.addEventListener("click", cerrarSolicitudGrua);
  overlay.innerHTML = `
    <section class="asClickCustomModal" role="dialog" aria-modal="true" aria-labelledby="solicitudGruaTitulo">
      <button type="button" class="asClickCustomClose" onclick="cerrarSolicitudGrua()" aria-label="Cerrar">✕</button>
      <div class="asClickCustomIcon">🚛</div>
      <h2 id="solicitudGruaTitulo">Solicitud de Grúa</h2>
      <p class="asClickCustomIntro">Completa la información para enviar la unidad adecuada.</p>
      <form id="solicitudGruaForm" onsubmit="event.preventDefault(); confirmarSolicitudGrua();">
        <label class="asClickFieldLabel" for="gruaDestino">Destino del vehículo *</label>
        <input id="gruaDestino" class="asClickTextInput" type="text" maxlength="250" placeholder="Escribe la dirección o lugar de destino" required>
        <fieldset class="asClickFieldset">
          <legend>¿Cuál es el estado del vehículo? *</legend>
          <div class="asClickOptionList">
            <label class="asClickOption"><input type="radio" name="gruaCondicion" value="Chocado"><span>🚗</span><b>Chocado</b></label>
            <label class="asClickOption"><input type="radio" name="gruaCondicion" value="Descompuesto"><span>🔧</span><b>Descompuesto</b></label>
          </div>
        </fieldset>
        <fieldset class="asClickFieldset">
          <legend>¿El vehículo está liberado? *</legend>
          <div class="asClickOptionList asClickTwoColumns">
            <label class="asClickOption"><input type="radio" name="gruaLiberado" value="Sí"><span>✅</span><b>Sí</b></label>
            <label class="asClickOption"><input type="radio" name="gruaLiberado" value="No"><span>⛔</span><b>No</b></label>
          </div>
        </fieldset>
        <fieldset class="asClickFieldset">
          <legend>¿El vehículo tiene carga? *</legend>
          <div class="asClickOptionList asClickTwoColumns">
            <label class="asClickOption"><input type="radio" name="gruaTieneCarga" value="No" onchange="actualizarCamposCargaGrua()"><span>➖</span><b>No</b></label>
            <label class="asClickOption"><input type="radio" name="gruaTieneCarga" value="Sí" onchange="actualizarCamposCargaGrua()"><span>📦</span><b>Sí</b></label>
          </div>
        </fieldset>
        <div id="camposCargaGrua" class="asClickLoadFields" hidden>
          <label class="asClickFieldLabel" for="gruaTipoCarga">Tipo de carga *</label>
          <input id="gruaTipoCarga" class="asClickTextInput" type="text" maxlength="150" placeholder="Ejemplo: material, mercancía o herramienta">
          <label class="asClickFieldLabel" for="gruaPesoCarga">Peso aproximado *</label>
          <div class="asClickInputSuffix">
            <input id="gruaPesoCarga" class="asClickTextInput" type="number" min="1" step="1" inputmode="numeric" placeholder="Ejemplo: 500">
            <span>kg</span>
          </div>
        </div>
        <p id="solicitudGruaError" class="asClickFormError" aria-live="polite"></p>
        <div class="asClickCustomActions">
          <button type="button" class="asClickSecondaryButton" onclick="cerrarSolicitudGrua()">Cancelar</button>
          <button type="submit" class="asClickPrimaryButton">Solicitar grúa</button>
        </div>
      </form>
    </section>
  `;
...
function construirMensajeServicio(
  ...
  return [
    ...
    `Ubicación: ${ubicacion}`,
    ...(servicioNormalizado === "grua" && datosGrua
      ? [
          "",
          "*DATOS PARA EL SERVICIO DE GRÚA*",
          `Destino: ${datosGrua.destino || "No registrado"}`,
          `Estado del vehículo: ${datosGrua.condicion || "No registrado"}`,
          `Vehículo liberado: ${datosGrua.liberado || "No registrado"}`,
          `Tiene carga: ${datosGrua.tieneCarga || "No registrado"}`,
          ...(datosGrua.tieneCarga === "Sí"
            ? [
                `Tipo de carga: ${datosGrua.tipoCarga || "No registrada"}`,
                `Peso aproximado: ${datosGrua.pesoCarga || "No registrado"} kg`
              ]
            : [])
        ]
      : []),
...
========================================= */
async function guardarSolicitudServicio(servicio, tipoTarifa, ubicacion, detalleServicio = "", datosGrua = null) {
  ...
  const datosGruaGuardados = datosGrua
    ? {
        destino: datosGrua.destino || "",
        condicion: datosGrua.condicion || "",
        liberado: datosGrua.liberado || "",
        tieneCarga: datosGrua.tieneCarga || "",
        tipoCarga: datosGrua.tipoCarga || "",
        pesoCarga: datosGrua.pesoCarga || ""
      }
    : null;
  const datosCliente = {
  ...
  try {
    const documentoServicio = await firestoreAddDoc(
      firestoreCollection(db, "servicios"),
      {
        ...
        ubicacionDatos: {
          ...
          enlaceGoogleMaps: enlaceUbicacion
        },
    ...
    try {
      await firestoreAddDoc(
        firestoreCollection(db, "solicitudes"),
        {
          ...
          tipoTarifa,
          costoServicio: obtenerCostoServicio(servicio),
          servicio: {
            nombre: servicio,
            tipo: tipoProveedor,
            tipoAuxilio: detalleServicio || ""
    ...
    } catch (errorSolicitudProveedor) {
      console.error(
        "El servicio se guardó en el historial, pero no fue posible enviarlo a proveedores:",
        errorSolicitudProveedor
      );
    }
    // Actualiza el historial en segundo plano para no retrasar
    // la apertura de WhatsApp ni la pantalla de seguimiento.
    cargarHistorialServicios().catch(errorHistorial => {
      console.warn("No fue posible refrescar el historial en segundo plano:", errorHistorial);
    });
    return folio;
  } catch (error) {
...
function obtenerUbicacion() {
  return new Promise(resolve => {
    const resultadoSinUbicacion = mensaje => ({
      latitud: null,
      longitud: null,
      precision: null,
      enlaceGoogleMaps: "",
    ...
    const guardarMejorPosicion = posicion => {
      const precision = Number(
        posicion.coords.accuracy || Infinity
      );
      const antiguedad =
        Date.now() -
        Number(posicion.timestamp || Date.now());
      if (antiguedad > 60000) return;
      if (
        !mejorPosicion ||
        precision <
          Number(
      ...
      if (precision <= 50) {
        finalizar(crearUbicacion(posicion));
      }
    };
    const mensajeError = error => {
      ...
      if (error?.code === 1) {
        return "No fue posible acceder a la ubicación. Activa el permiso y la ubicación precisa para este navegador";
      }
      if (error?.code === 2) {
    ...
    watchId = navigator.geolocation.watchPosition(
      guardarMejorPosicion,
      error => {
        if (mejorPosicion) {
          finalizar(
            crearUbicacion(mejorPosicion)
          );
          return;
        }
        if (error?.code === 1) {
...
async function activarAlerta(tipo) {
  ...
  try {
    const ubicacion = await obtenerUbicacion();
    const folio = generarFolioEmergencia();
    const nombrePublico = String(
      perfilActual.nombre || "Miembro"
    )
      .trim()
      .split(/\s+/)[0];
    const serieCompleta = String(
      perfilActual.serie || ""
    ).trim();
    const seriePublica = serieCompleta
      ? serieCompleta.slice(-6)
      : "No registrada";
    const referenciaAlerta = await firestoreAddDoc(
      firestoreCollection(db, "alertasEmergencia"),
      {
        folio,
        tipo,
        ...
        usuarioId: usuarioActual?.uid || "",
        uid: usuarioActual?.uid || "",
        numeroMembresia: perfilActual.numeroMiembro || "",
        cliente: {
    ...
    const enlaceSeguimiento = generarEnlaceSeguimiento(referenciaAlerta.id);
    await firestoreUpdateDoc(
      firestoreDoc(db, "alertasEmergencia", referenciaAlerta.id),
      {
        idAlerta: referenciaAlerta.id,
        enlaceSeguimiento,
        actualizadoEn: firestoreServerTimestamp()
      }
    );
    const ubicacionPublica =
      ubicacion?.enlaceGoogleMaps ||
      ubicacion?.mensaje ||
      "Ubicación no disponible";
    const mensaje = [
      ...
      `Serie: ${seriePublica}`,
      "",
      `Ubicación actual: ${ubicacionPublica}`,
      "",
      "*SEGUIMIENTO DEL CASO*",
      enlaceSeguimiento,
      "",
      "Desde este enlace se podrán registrar actualizaciones como:",
      "• Voy para allá",
      "• Llegó la policía",
      "• Cliente está bien",
      "• Vehículo localizado",
    ...
    console.error("No fue posible activar la alerta:", error);
    mostrarModal(
      "⚠",
      "No fue posible activar la alerta",
      "Revisa tu conexión, permite la ubicación e inténtalo nuevamente."
    );
  }
}
function generarFolioEmergencia() {
  const fecha = new Date();
  const anio = fecha.getFullYear().toString().slice(-2);
  const mes = String(fecha.getMonth() + 1).padStart(2, "0");
  const dia = String(fecha.getDate()).padStart(2, "0");
  ...
  return `ASC-EM-${anio}${mes}${dia}-${hora}${minuto}-${aleatorio}`;
}
function generarEnlaceSeguimiento(idAlerta) {
  const rutaActual = window.location.pathname;
  const carpetaBase = rutaActual.substring(0, rutaActual.lastIndexOf("/") + 1);
  return `${window.location.origin}${carpetaBase}seguimiento.html?id=${encodeURIComponent(idAlerta)}`;
}
/* =========================================
   WHATSAPP ASESOR
========================================= */
...
function generarEnlaceSeguimientoServicio(folio) {
  const rutaActual = window.location.pathname;
  const carpetaBase =
    rutaActual.substring(
      0,
      rutaActual.lastIndexOf("/") + 1
    );
  return (
    `${window.location.origin}${carpetaBase}` +
...
function agregarEstilosFuncionesNuevas() {
  ...
  estilos.textContent = `
    ...
    .asClickFieldLabel{display:block;margin:14px 0 7px;font-weight:700;color:#173b63}
    .asClickTextInput{width:100%;box-sizing:border-box;border:1px solid #cad7e5;border-radius:9px;padding:12px 13px;font:inherit;color:#173b63;background:#fff}
    .asClickTextInput:focus{outline:2px solid rgba(23,109,220,.18);border-color:#176ddc}
    .asClickFieldset{border:0;padding:0;margin:20px 0 0}.asClickFieldset legend{font-weight:700;margin-bottom:9px;color:#173b63}
    .asClickTwoColumns{grid-template-columns:repeat(2,minmax(0,1fr))}
    .asClickLoadFields{margin-top:16px;padding:14px;border:1px solid #d9e3ee;border-radius:10px;background:#f7faff}
    .asClickInputSuffix{display:flex;align-items:center;gap:8px}.asClickInputSuffix .asClickTextInput{flex:1}.asClickInputSuffix span{font-weight:700;color:#526a82}
    .asClickFormError{min-height:20px;margin:12px 0 0;color:#c62828;font-size:13px;font-weight:700}
...
window.abrirSolicitudGrua = abrirSolicitudGrua;
window.actualizarCamposCargaGrua = actualizarCamposCargaGrua;
window.confirmarSolicitudGrua = confirmarSolicitudGrua;
window.cerrarSolicitudGrua = cerrarSolicitudGrua;
