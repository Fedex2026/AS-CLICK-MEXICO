...
const PRECIO_MEMBRESIA_AS_CLICK = 1000;

 

const PRECIOS_SERVICIOS_AS_CLICK = {

 

  miembro: { ajustador: 500, abogado: 900, "auxilio vial": 190 },

 

  publico: { ajustador: 1800, abogado: 2400, "auxilio vial": 380 }
...
function obtenerCostoServicioCliente(servicio) {

 

  const tipo = String(servicio || "").trim().toLowerCase().normalize("NFD")
  ...
  if (tipo === "grua") return null;

 

  const esMiembro = perfilActual?.tieneMembresia === true && perfilActual?.estadoMembresia === "activa";

 

  return (esMiembro ? PRECIOS_SERVICIOS_AS_CLICK.miembro : PRECIOS_SERVICIOS_AS_CLICK.publico)[tipo] ?? null;
...
function formatearCostoServicioCliente(servicio) {

 

  const costo = obtenerCostoServicioCliente(servicio);

 

  return costo == null ? "A cotizar" : `$${Number(costo).toLocaleString("es-MX")} MXN`;
...
function actualizarPreciosServiciosCliente() {

 

  const precios = { precioServicioAjustador:"Ajustador", precioServicioAbogado:"Abogado", precioServicioAuxilio:"Auxilio vial", precioServicioGrua:"Grúa" };

 

  Object.entries(precios).forEach(([id, servicio]) => { const el=document.getElementById(id); if(el) el.textContent=formatearCostoServicioCliente(servicio); });

 

  const membresia=document.getElementById("precioMembresiaAsClick");

 

  if(membresia) membresia.textContent=`$${PRECIO_MEMBRESIA_AS_CLICK.toLocaleString("es-MX")} MXN`;
...
// Solo para el flujo de Grúa: conserva la ubicación GPS actual mientras se cotiza.

let ubicacionGruaActual = "";
...
function construirPerfilDashboard(user, datos) {
  ...
  return {
    ...
    numeroEstampaMontec: datos.numeroEstampaMontec || "",
...
function actualizarDatosPantalla(perfil) {
  ...
  actualizarPreciosServiciosCliente();
...
function actualizarAfiliacionesPantalla(perfil) {
  ...
  const logoMontec = document.getElementById("logoMontec");
  ...
  if (!contenedor || !logoMontec || !logoAcme) return;
  ...
  const seleccion = String(perfil?.afiliacionGremial || "").trim().toLowerCase();
  ...
  const montecActivo =
    ...
    afiliaciones?.montec?.activo === true ||
  ...
  logoMontec.hidden = !montecActivo;
  ...
  contenedor.hidden = !(montecActivo || acmeActivo);
...
function prepararUbicacionActualGrua() {
  ...
  let box = document.getElementById("gruaUbicacionActualBox");

  if (!box) {

    box = document.createElement("div");

    box.id = "gruaUbicacionActualBox";

    box.style.cssText = "margin:0 0 14px;padding:12px 14px;border:1px solid rgba(59,130,246,.35);border-radius:12px;background:rgba(15,23,42,.55);";

    box.innerHTML = `

      <div style="font-weight:800;margin-bottom:6px;">📍 Ubicación actual</div>

      <div id="gruaUbicacionActualTexto" style="font-size:13px;opacity:.85;margin-bottom:8px;">Obteniendo ubicación...</div>

      <button id="gruaActualizarUbicacionBtn" type="button" style="border:0;border-radius:9px;padding:8px 11px;font-weight:700;cursor:pointer;">Actualizar ubicación</button>
    ...
    const destinoContenedor = destino?.closest(".formGroup, .gruaFormGroup, .field, label") || destino?.parentElement;

    if (destinoContenedor?.parentNode) {

      destinoContenedor.parentNode.insertBefore(box, destinoContenedor);
    ...
    document.getElementById("gruaActualizarUbicacionBtn")?.addEventListener("click", () => {

      actualizarUbicacionActualGrua(true);
...
async function actualizarUbicacionActualGrua(mostrarError = false) {

  if (ubicacionGruaObteniendo) return ubicacionGruaActual;

 

  ubicacionGruaObteniendo = true;

  const texto = document.getElementById("gruaUbicacionActualTexto");

  const boton = document.getElementById("gruaActualizarUbicacionBtn");

  if (texto) texto.textContent = "Obteniendo ubicación actual...";
  ...
  try {
    ...
    const esMapa = /^https?:\/\/maps\.google\.com\/\?q=-?\d+(?:\.\d+)?,-?\d+(?:\.\d+)?/i.test(String(ubicacion || ""));
    ...
    if (!esMapa) {
      ...
      if (texto) texto.textContent = "No fue posible obtener tu ubicación. Activa la ubicación del navegador.";

      if (mostrarError) {

        const errorBox = document.getElementById("gruaFormError");

        if (errorBox) errorBox.textContent = "Para cotizar una grúa necesitamos tu ubicación actual.";
    ...
    ubicacionGruaActual = ubicacion;

    if (texto) texto.textContent = "Ubicación actual lista ✓";

    return ubicacionGruaActual;

  } finally {

    ubicacionGruaObteniendo = false;
...
function abrirCotizacionGrua() {
  ...
  prepararUbicacionActualGrua();

  actualizarUbicacionActualGrua(false);
...
async function enviarSolicitudCotizacionGrua(event) {
  ...
  try {
    ...
    const ubicacion = ubicacionGruaActual || await actualizarUbicacionActualGrua(true);

    if (!ubicacion) {

      if (errorBox) errorBox.textContent = "Activa tu ubicación para solicitar la cotización de grúa.";
...
async function guardarSolicitudCotizacionGrua(datosGrua) {
  ...
  await firestoreSetDoc(servicioRef, {
    ...
    destino: datosGrua.destino, destinoDatos, cliente, vehiculo, grua: detalleGrua,
...
async function autorizarCotizacionGrua(solicitudId, proveedorUid) {
  ...
  try {
    ...
    await firestoreRunTransaction(db, async transaction => {
      ...
      folioSeguimiento = solicitud.folio || solicitudId;
    ...
    mostrarModal("✓", "Cotización autorizada", "El proveedor elegido quedó asignado. Abriendo el seguimiento del servicio...");
    ...
    if (folioSeguimiento) {
      ...
      setTimeout(() => {
        ...
        window.location.href = `./servicio-seguimiento.html?folio=${encodeURIComponent(folioSeguimiento)}`;
...
function construirMensajeServicio(
  ...
  return [
    ...
    `Costo: ${costoCliente == null ? "A cotizar" : `$${Number(costoCliente).toLocaleString("es-MX")} MXN`}`,
...
async function guardarSolicitudServicio(servicio, tipoTarifa, ubicacion, detalleServicio = "", costoCliente = null) {
  ...
  try {
    ...
    if (resultado.empty && (perfilActual.marca || perfilActual.placas || perfilActual.serie)) {
      ...
      await firestoreSetDoc(referenciaInicial, {
        ...
        membresiaAplicada: perfilActual.tieneMembresia === true && perfilActual.estadoMembresia === "activa",
        ...
        estadoMembresiaVehiculo:
          ...
          perfilActual.tieneMembresia === true && perfilActual.estadoMembresia === "activa"
...
function renderizarVehiculos() {
  ...
  lista.innerHTML = vehiculosUsuario.map(vehiculo => {
    ...
    const estadoVehiculo = vehiculo.estadoMembresiaVehiculo ||
...
async function guardarNuevoVehiculo(event) {
  ...
  const coberturaSeleccionada =
    ...
    document.querySelector('input[name="nuevoVehiculoCobertura"]:checked')?.value ||
  ...
  const solicitaAutorizacion = coberturaSeleccionada === "solicitar_autorizacion";
  ...
  if (solicitaAutorizacion && !(perfilActual.tieneMembresia === true && perfilActual.estadoMembresia === "activa")) {
    ...
    if (error) {
      ...
      error.textContent = "Necesitas una membresía AS CLICK activa para solicitar beneficios para este vehículo.";
  ...
  try {
    ...
    const referencia = await firestoreAddDoc(
...

      {
        ...
        requiereAutorizacion: solicitaAutorizacion,
...
document.addEventListener("change", event => {
  ...
  const sinMembresia = event.target.value === "sin_membresia";
  ...
  const avisoAutorizacion = document.getElementById("vehicleAuthorizationNotice");
